test_that("classify_units() works", {
  ...
  data <- flexurba::preprocess_units(units, classification, pop)
  ...
  expect_error(flexurba::preprocess_units(sf::st_transform(units, 4326), classification, pop))

  # resolution of grids should be multiple of resample resolution
  expect_error(flexurba::preprocess_units(units, classification, pop, resample_resolution = 58))

  # invalid dissolve_units_by parameter
  expect_error(flexurba::preprocess_units(units, classification, pop, dissolve_units_by = "ERROR"))
