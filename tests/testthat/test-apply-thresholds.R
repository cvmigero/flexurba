test_that("apply_threshold() and convert_regions_to_grid() works", {
  ...
  expect_equal(length(unique(terra::values(convert_regions_to_grid(units, proxies$pop)))), 580)
  ...
  expect_equal(sum(terra::values(pop_above_1500$rboundaries), na.rm = TRUE), 3920)

  built_above_20 <- apply_threshold(proxies$built, type = "predefined", threshold_value = 0.20)
  expect_equal(sum(terra::values(built_above_20$rboundaries), na.rm = TRUE), 3573)
  ...
  expect_equal(sum(terra::values(pop_above_p95$rboundaries), na.rm = TRUE), 3620)

  built_above_mean <- apply_threshold(proxies$built, type = "data-driven", fun = "mean")
  expect_equal(sum(terra::values(built_above_mean$rboundaries), na.rm = TRUE), 23728)

  light_above_p75 <- apply_threshold(proxies$light, type = "data-driven", fun = "p75")
  expect_equal(sum(terra::values(light_above_p75$rboundaries), na.rm = TRUE), 18304)
