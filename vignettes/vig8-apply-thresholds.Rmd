...
We check the data-driven threshold values using the code below. In Egypt, the 99th percentile value is 2083.6820 inhabitants per km². The value for Sudan is only 282.5375 inhabitants per km².
...
The delineations generated by the four code examples are displayed below. Comparing the absolute-predefined approach (top left) with the relative-predefined approach (bottom left), we observe that the latter identifies more urban areas in Sudan because of the less strict threshold in that country. Furthmore, when the data-driven threshold is applied in an absolute manner (using data from both Egypt and Sudan; top right) almost the entire Nile Delta is classified as urban. However, this is not the case when the threshold is tailored separately for each country.
