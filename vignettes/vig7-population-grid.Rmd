---
title: "7. Classification with other population grids"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{7. Classification with other population grids}
  %\VignetteEngine{knitr::rmarkdown}
  %\VignetteEncoding{UTF-8}
---

This vignette illustrates how to use the `flexurba` functionalities with other population grids. The package standard uses the [GHSL-POP grid](https://human-settlement.emergency.copernicus.eu/ghs_pop2023.php) to generate an alternative version of the DEGURBA classification (with the function `download_GHSLdata()`), but any other population grid can be used in principle. In this vignette, we employ the [*Worldpop constrained UN adjusted population grid of Ghana*](https://hub.worldpop.org/geodata/summary?id=49691) with a resolution of 100m as an example (Bondarenko et al., 2020).
...
worldpop_ghana <- terra::rast(system.file('extdata', 'data-ghana', 'gha_ppp_2020_UNadj_constrained.tif', package='flexurba'))
```
...
As mentioned earlier, the `flexurba` functions expect a population grid in the Mollweide projection system. Let's check the coordinate system of the data and change it to Mollweide as follows.
...
```{r, echo=FALSE, results=FALSE}
built_grid <- terra::rast(system.file('extdata', 'data-ghana', 'BUILT_S.tif', package='flexurba'))
land_grid <- terra::rast(system.file('extdata', 'data-ghana', 'LAND.tif', package='flexurba'))
```
...
Great! Then, we save the preprocessed population grid as follows.
...
Now that we have the population grid in the right format, we can use it to construct the grid cell classification of DEGURBA. The function `classify_grid()` reads the `POP.tif` (which contains our population grid), `BUILT_S.tif` and `LAND.tif` from the `data-ghana` folder and constructs the grid classification.

```{r, eval=FALSE}
# construct classification
classification1 <- classify_grid('data-ghana')

# plot the classification
DoU_plot_grid(classification1)
```

![](figures/ghana2.png){alt="Ghana classification 2"}
...
classification2 <- classify_grid('data-ghana', 
                                parameters = c(
