...
Next, we can execute the grid cell classification with the function `classify_grid()`. The `parameters` in the function should contain the specification as described below to precisely reconstruct the results of the two different Data Packages.
...
classification2022 <- classify_grid(
...
-   The 'optimal' built-up area criterium is reproduced by the following two parameter settings:
    -   `UC_built_threshold = "optimal"` indicates that an "optimal" built-up area threshold should be determined. This 'optimal' threshold, as defined in [Data Package 2023](https://ghsl.jrc.ec.europa.eu/documents/GHSL_Data_Package_2023.pdf), is calculated as the average built-up area proportion in clusters of cells with a density of at least 1500 inhabitants per km² and a minimum total population of 5000 people. The `flexurba` function `get_optimal_builtup()` is employed for this calculation.
...
classification2023 <- classify_grid(
...
For more information about all possible parameter settings of the reader can consult the section 'Custom specification' in the documentation of the function `classify_grid()`.
...
classification_builtup2022 <- classify_grid(
  data = "data/belgium2023",
...
# (Note that determining the "optimal" built-up area 
# from the global data can take some time)
classification_builtup2023 <- classify_grid(
  data = "data/belgium2023",
  parameters = list(
    UC_built_threshold = "optimal",
    built_optimal_data = "data/global2023"
...
For the data of epoch 2020 in Data Package 2023, we can determine the exact value of this 'optimal' threshold with the function `get_optimal_builtup()`:

```{r, eval=FALSE}
# (Note that determining the "optimal" built-up area 
# from the global data can take some time)
get_optimal_builtup("data/global2023",
