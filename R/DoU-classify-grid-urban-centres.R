...
DoU_classify_grid_urban_centres <- function(data, density_threshold = 1500, size_threshold = 50000, contiguity_rule = 4, built_criterium = TRUE, built_threshold = 0.2, smooth_pop = FALSE, smooth_pop_window = 5, gap_fill = TRUE, max_gap = 15, smooth_edge = TRUE, smooth_edge_fun = "majority_rule_R2023A", value = 3) {
  ...
  if (!built_criterium %in% c(TRUE, FALSE)) {
    stop(paste("Invalid argument:", built_criterium, "is not a valid parameter for built_criterium"))
  }
  
  if (is.na(as.numeric(built_threshold))) {
    stop(paste("Invalid argument:", built_threshold, "is not a valid parameter for built_threshold"))
  } else {
  ...
  if (!(gap_fill %in% c(TRUE, FALSE))) {
    stop(paste("Invalid argument:", gap_fill, "is not a valid parameter for gap_fill"))
  }
  
  if (!smooth_edge %in% c(TRUE, FALSE)) {
    stop(paste("Invalid argument:", smooth_edge, "is not a valid parameter for smooth_edge"))
  }
  
  if (!((is.function(smooth_edge_fun)) || (smooth_edge_fun %in% c("majority_rule_R2022A", "majority_rule_R2023A")))) {
    stop(paste("Invalid argument:", smooth_edge_fun, "is not a valid parameter for smooth_edge_fun"))
  }
  ...
  if (built_criterium) {
    urbancentres <- flexurba::get_clusters(
      ...
      xden2 = data$built_per_land, minden2 = built_threshold,
