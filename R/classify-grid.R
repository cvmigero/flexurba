...
#'
#'     Path to the directory with the data, or named list with the data as returned by function [preprocess_grid()] used to determine the optimal built threshold (see section "Built-up area criterium" below). Ignored when `UC_built_criterium` is `FALSE` or when `UC_built_threshold` is not `"optimal"`.
